...
class RegisterPage extends RunnerPage
{
	...
	protected function activateNewUser()
	{
		...
		$sql = "select ".$this->getFieldSQLDecrypt("password")
			." from ". $this->connection->addTableWrappers( "online_placard_users" ) 
			." where ".
		...
		$sql = "update ". $this->connection->addTableWrappers( "online_placard_users" )
	...
	if( GetGlobalData("userRequireActivation") ) {
		...
		$retval = DoInsertRecord("online_placard_users", $values, $blobfields, $this);
		if( GetGlobalData("userRequireActivation") ) {
	...
	protected function checkIfUsernameUnique( $strUsername )
	{
		...
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "online_placard_users" ) 
			. " where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("email"), 
				$sUsername,
