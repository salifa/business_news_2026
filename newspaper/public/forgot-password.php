...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT ID, email, fullname FROM online_placard_users WHERE email = ? AND active = 1");
            $stmt->execute([$email]);
            ...
            if ($user) {
                ...
                $stmt = $db->prepare("
                    ...
                    SET reset_token = ?, reset_date = ? 
                    WHERE email = ?
                ");
                $stmt->execute([$token, $expiry, $email]);
                
                // Send reset email
                $resetLink = BASE_URL . "public/reset-password.php?token=$token";
                ...
                <body>
                    <div class='container'>
                        ...
                        <div class='content'>
                            ...
                            <p>หากคุณไม่ได้ร้องขอการรีเซ็ตรหัสผ่าน กรุณาเพิกเฉยอีเมลนี้</p>
                        </div>
                        <div class='footer'>
                            <p>" . NEWSPAPER_NAME . "</p>
                ...
                $message = 'ส่งลิงก์รีเซ็ตรหัสผ่านไปยังอีเมลของคุณแล้ว หากอีเมลนี้มีอยู่ในระบบ';
                $messageType = 'success';
            }
...
<head>
    ...
    <title>ลืมรหัสผ่าน - <?php echo NEWSPAPER_NAME; ?></title>
