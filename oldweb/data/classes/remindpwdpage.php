...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( $this->remindMethod() == RPM_RESET )
		{
			...
			$this->connection->exec("update ".$this->connection->addTableWrappers("online_placard_users")." set "
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "online_placard_users" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "online_placard_users" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "online_placard_users" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
		...
		$selectClause.= ", ".$this->connection->addFieldWrappers("active");
		
		$strSQLData = array();
		$strSQLData["where"] = $sWhere;		
		$strSQLData["sql"] = $selectClause." from ".$this->connection->addTableWrappers("online_placard_users")." where ".$sWhere;
