...
Runner.pages.PageSettings.addPageEvent('placardpdf', Runner.pages.constants.PAGE_ADD, "afterPageReady", function(pageObj, proxy, pageid, inlineRow, inlineObject, row ) {
...
var meetingCount = Runner.getControl(pageid,'count'); // set current year
...
// set the count field to current year

year = "___/"+ ( myToday.getFullYear() + 543 );
meetingCount.setValue( year );
 ...
 */
Runner.pages.fieldsEvents["meeting_date_event"] = function( pageObj, pageid, row ) {
	if ( row ) {
	...
	var ret, reqParams,
		...
		fieldsData = {},
		params = {},
		ajax = new Runner.form.Button({ id: 'meeting_date_event', btnName: 'meeting_date_event' }),
		before = function() {
			//Moment.js date object

//alert ("onchange");

var ctrl = Runner.getControl(pageid, 'placard_date');
var placardDate = ctrl.getValue();
//alert (placardDate);
//exit;
var meetingdate = new Date();
/// meetingdate.setDate(placardDate.getDate() + 8 ); //cast the variable and add date
//alert (meetingdate) ;
// exit; 
//var end_moment_date = ctrl.getDateValue().add(365,'days');

//var js_start = new Date(new Date().getTime());  // start today
//js_start.setHours (00,00,0);

var mtDateStr = meetingdate.getFullYear()+"-" + (meetingdate.getMonth()+1)+ "-" + (meetingdate.getDate());  // format date

//alert (mtDateStr);

var ctrl2 = Runner.getControl(pageid, 'meeting_date');
/// ctrl2.setValue(mtDateStr); // reset  value ( don't know why)
/// ctrl2.setAllowedInterval( mtDateStr, null, '8 วันขื้นไป ' );
