...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="payment" && $field=="id")
	{
		$value = "<a href=\"https://www.vnnbiz.com/data/?".$data["id"]."\" title= \"แจ้งชำระแงิน\" class= \"rnr-button-img\" data-icon=\"print\"> </a>";;
		return $value;
	}
				if($table=="placard_download" && $field=="id")
	{
		$value = "<a href=\"/lic/Design/index2.php?print_date=".$data["placard_date"]."\" title= \" พิมพ์   \" class= \"rnr-button-img\" data-icon=\"print\" target=\"_blank\"> Download </a>";;
		return $value;
	}
	return $value;
}
...
{
	$value = "";
				if($table=="payment" && $field=="slip_upload")
	{
		;
		return $value;
	}
				if($table=="payment admin" && $field=="slip_upload")
	{
		;
		return $value;
	}
				if($table=="payment slip" && $field=="slip_upload")
	{
		;
		return $value;
	}
				if($table=="placard admin" && $field=="pdf_file1")
	{
		;
		return $value;
	}
				if($table=="credit_admin" && $field=="slipup_upload")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="placard" && $field=="owner_id")
	{
		return $_SESSION["UserID"];
	}
				if($table=="placard" && $field=="companyname")
	{
		return "บริษัท ... จำกัด";
	}
				if($table=="placard" && $field=="count")
	{
		return "1/2563";
	}
				if($table=="placard" && $field=="placard_to")
	{
		return ท่านผู้ถือหุ้นของบริษัท;
	}
				if($table=="placard" && $field=="meeting_time")
	{
		return "10:30";
	}
				if($table=="placard" && $field=="position")
	{
		return กรรมการผู้มีอำนาจลงนาม;
	}
				if($table=="credit_transactions" && $field=="owner_id")
	{
		return $_SESSION["UserID"];
	}
				if($table=="credit_transactions" && $field=="buy_timestamp")
	{
		return now();
	}
				if($table=="payment" && $field=="owner_id")
	{
		return $_SESSION["UserID"];
	}
				if($table=="payment" && $field=="paydate")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="payment" && $field=="paytime")
	{
		return strftime("%H:%M:%S");
	}
				if($table=="payment" && $field=="amount")
	{
		return 0;
	}
				if($table=="payment admin" && $field=="paytime")
	{
		return now();
	}
				if($table=="payment slip" && $field=="owner_id")
	{
		return $_SESSION["UserID"];
	}
				if($table=="payment slip" && $field=="paytime")
	{
		return now();
	}
				if($table=="placard admin" && $field=="companyname")
	{
		return "บริษัท ... จำกัด";
	}
				if($table=="placard admin" && $field=="count")
	{
		return "1/2563";
	}
				if($table=="placard admin" && $field=="placard_to")
	{
		return ท่านผู้ถือหุ้นของบริษัท;
	}
				if($table=="placard admin" && $field=="meeting_time")
	{
		return "10:30";
	}
				if($table=="placard admin" && $field=="position")
	{
		return กรรมการผู้มีอำนาจลงนาม;
	}
				if($table=="placard admin" && $field=="create_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="credit_admin" && $field=="owner_id")
	{
		return $_SESSION["UserID"];
	}
				if($table=="credit_admin" && $field=="buy_timestamp")
	{
		return now();
	}
				if($table=="placard_download" && $field=="companyname")
	{
		return "บริษัท ... จำกัด";
	}
	return "";
